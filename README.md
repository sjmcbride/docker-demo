...
```
demo1.smclab.net → nginx-proxy:443 → demo1:80 → PHP → PostgreSQL:5432
demo2.smclab.net → nginx-proxy:443 → demo2:80 → PHP → PostgreSQL:5432
demo3.smclab.net → nginx-proxy:443 → demo3:80 → PHP → PostgreSQL:5432
demo4.smclab.net → nginx-proxy:443 → demo4:80 → PHP → PostgreSQL:5432
```
...
| `./demo[1-4]` | Site source code |
| `./database` | DB initialization |

### Security

- **Network**: Isolated Docker bridge, only nginx-proxy exposed
- **SSL**: Automatic Let's Encrypt certificates with auto-renewal
- **Database**: Internal-only access, session-based authentication
